...
def importSpreadsAndFinalScores(event, context):
    ...
    FBP_SCHEDULE_TABLE = os.environ.get('FBPScheduleTableName')
    ...
    try:
        ...
        for spread in spreads_data:
            ...
            winner=spread['Winner']  ## Can be either "H" or "A"
            home=spread['Home']
            away=spread['Away']
            date=spread['Date']
            ##
            ...
            try:     
                table.update_item(
                    ...
                    UpdateExpression="SET #spread = :spread, #underdog = :underdog, #home = :home, #away = :away, #homeScore = :homeScore, #awayScore = :awayScore, #winner = :winner, #date = :date",
                    ExpressionAttributeNames={"#spread": "Spread", "#underdog": "Underdog", "#home": "Home", "#away": "Away", "#homeScore": "HomeScore", "#awayScore": "AwayScore", "#winner": "Winner", "#date": "Date"},
                    ExpressionAttributeValues={":spread": spread['Spread'], ":underdog": underdog, ":home": home, ":away": away, ":homeScore": spread['HomeScore'], ":awayScore": spread['AwayScore'], ":winner": winner, ":date": date}
                )
            except ClientError as e:
                error_msg = e.response.get('Error', {}).get('Message', str(e))
                logger.exception(f"Failed to insert spread data into DynamoDB for game: {spread['Home']} vs {spread['Away']}. Error: {error_msg}")
                fbpLog("fbpadmin@my-fbp.com", "ImportSpreads", f"Failed to insert spread data into DynamoDB for game: {spread['Home']} vs {spread['Away']}. Error: {error_msg}", "ERROR")
            except Exception as e:
                logger.exception(f"Unexpected error while processing spread data for game: {spread['Home']} vs {spread['Away']}. Error: {str(e)}")
                fbpLog("fbpadmin@my-fbp.com", "ImportSpreads", f"Unexpected error while processing spread data for game: {spread['Home']} vs {spread['Away']}. Error: {str(e)}", "ERROR")
        ...
        ##
        archive_key = f"schedule/{os.environ.get('Year')}-Schedule/archive/week{week}-schedule.csv"
        try:
        ...
        error_msg = e.response.get('Error', {}).get('Message', str(e))
        logger.exception(f"Failed to list objects in S3 bucket: {bucket_name}. Error: {error_msg}")
        fbpLog("fbpadmin@my-fbp.com", "ImportSpreads", f"Failed to list objects in S3 bucket: {bucket_name}. Error: {error_msg}", "ERROR")
    return {
        'statusCode': 500,
        'body': json.dumps({
